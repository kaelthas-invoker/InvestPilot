"""InvestPilot smoke test: 验证 fin-agent CLI 真实可跑."""
...
def test_fin_agent_status_runs_via_subprocess() -> None:
    """fin-agent status 通过子进程跑应退出码 0 且输出含 InvestPilot v0.1."""
    result = subprocess.run(
        [sys.executable, "-m", "investpilot.cli.main", "status"],
        capture_output=True,
        text=True,
        timeout=30,
    )
    ...
    assert "InvestPilot v0.1" in lines[0], f"第 1 行缺少版本: {lines[0]!r}"
...
def test_fin_agent_status_via_cli_runner(runner: CliRunner) -> None:
    ...
    assert "InvestPilot v0.1" in lines[0]
    assert lines[1].startswith("20")
    assert lines[2].startswith("Python 3.")


def test_fin_agent_help_via_cli_runner(runner: CliRunner) -> None:
...
def test_package_version() -> None:
    """investpilot.__version__ 应等于 '0.1.0'."""
    import investpilot

    assert investpilot.__version__ == "0.1.0"
